...
foreach ($repo in $repos) {
    ...
    if ($repoName.EndsWith(".md")) { $repoName = $repoName.Replace(".md", "") } # Handle weird names
    
    if (!(Test-Path $repoName)) {
        Write-Host "Starting job for $repo..."
        $jobs += Start-Job -ScriptBlock {
            param($r, $n)
            $env:GIT_TERMINAL_PROMPT = 0
            git clone --depth 1 $r $n 2>&1 | Out-Null
        } -ArgumentList $repo, $repoName
    } else {
...
Write-Host "Jobs done or timed out. Cleaning up jobs..."
$jobs | Remove-Job -Force
Write-Host "All cloning jobs handled."
